...
#combined RSS
#print(paste0(" Combined RSS = ", RSS.nymph+RSS.adult+RSS.egg))
...
# 1989
#Optim.result  <- 0.174609995299276

# 1990
#Optim.result  <- 7.2480670766423713e-2
...
# 1989
#day.of.start <- round(20.0009480972384 , 0 )

#day.of.start.no.age <- round( 20.012484066155359, 0 ) #<-- Careful !

# 1990
day.of.start <- round(17.050244731737820 , 0 ) 

#day.of.start.no.age <- round( 17.4339503553022, 0 ) <-- Careful !
...
# 
# plot( simulation.output$day, simulation.output$egg,
#       lty=1, type="l", lwd = 4,
#       xlab= "Days from 18 November 1990",
#       ylab= "Total Egg (#)",
#       xlim =c(1,max(simulation.output$day)+20),
#       ylim =c(0,max(simulation.output$egg)+200),
#       col="black",
#       cex.lab=2,
#       frame.plot = FALSE,
#       axes=FALSE,
...
plot( simulation.output$day, simulation.output$nymph,
      lty=1, type="l", lwd = 6,
      xlab= "Days from 17 November 1990",
      ylab= "Total Nymph (#)",
      xlim =c(1,max(simulation.output$day)+20),
      ylim =c(0,max(simulation.output$nymph)+100),
      col="black",
      cex.lab=2,
      frame.plot = FALSE,
      axes=FALSE,
...
plot( simulation.output$day, simulation.output$adult,
      lty=1, type="l", lwd = 6,
      xlab= "Days from 17 November 1990",
      ylab= "Total Adult (#)",
      xlim =c(1,max(simulation.output$day)+20),
      ylim =c(0,max(simulation.output$adult)+15),
      col="black",
      cex.lab=2,
      frame.plot = FALSE,
      axes=FALSE,
...
# Nymphs developmental rate 

Nymphs.developmental.rate <- function(temperature){
...
Lower_Dev_Threshold = 11.78421
  
Nymph_Devel_Rate = exp(p*temperature) - exp( (p*temperature) - (k-temperature)/delta) + lambda
  
  if(temperature < Lower_Dev_Threshold) {Nymph_Devel_Rate <- 0.0}
  
  if(temperature > Upper_Dev_Threshold) {Nymph_Devel_Rate <- 0.0}
  
  return(Nymph_Devel_Rate)
...
Pupa.developmental.rate <- function(temperature){
...
Lower_Dev_Threshold = 5.228402
  
Pupa_Devel_Rate = exp(p*temperature) - exp((p*temperature) - ((k-temperature)/delta) ) + lambda
  
  if(temperature < Lower_Dev_Threshold) {Pupa_Devel_Rate <- 0.0}
  
  if(temperature > Upper_Dev_Threshold) {Pupa_Devel_Rate <- 0.0}
  
  return(Pupa_Devel_Rate)
...
# date <- c("1989-01-01", "1989-11-20") # day.of.start = 20

# 01 Nov 1990 to 31 Dec 1991
#date <- c("1990-10-01", "1990-11-17") # day.of.start = 17
...
# 1988
r0_Egg    <- 0.000000000000000
r0_Nymph  <- 0.486067420789637
r0_Pupa   <- 0.551074099697934

# 1989
#r0_Egg    <- 0.625394985562994
#r0_Nymph  <- 0.000000000000000
#r0_Pupa   <- 0.320244584241091

# 1990
#r0_Egg    <- 0.175797397471555
#r0_Nymph  <- 0.873278098638999
#r0_Pupa   <- 0.000000000000000


date.age.zero.pupa <- function( r0_Pupa , last_date ){

if(r0_Pupa > 0 ){  
  
  # decrease the pupae decrease of the 
  repeat{
    
    last_date <- last_date - 1
    
    r0_Pupa <- r0_Pupa - Pupa.developmental.rate(weather$T2M[last_date])
...
date.age.zero.nymph <- function( r0_Nymph , last_date ){
  
if(r0_Nymph > 0){
# decrease the nymph decrease of the 
repeat{
  
  last_date <- last_date - 1
  
  r0_Nymph <- r0_Nymph - Nymphs.developmental.rate(weather$T2M[last_date])
...
date.age.zero.egg <- function( r0_Egg, last_date ){
  
if(r0_Egg > 0){
  # decrease the egg decrease of the 
  repeat{
    
    last_date <- last_date - 1
    
    r0_Egg <- r0_Egg - Eggs.developmental.rate(weather$T2M[last_date])
...
pupa_zero <- function (r0_Pupa , last_date){

if(r0_Pupa > 0 ){
  
last_date  <- date.age.zero.pupa ( r0_Pupa , last_date )

r0_Nymph <- 1.0

last_date <- date.age.zero.nymph ( r0_Nymph , last_date )

r0_Egg <- 1.0

last_date   <- date.age.zero.egg ( r0_Egg, last_date)
...
nymph_zero <- function (r0_Nymph , last_date){
...
p0 <- pupa_zero(r0_Pupa , last_date)

n0 <- nymph_zero(r0_Nymph , last_date)

e0 <- date.age.zero.egg( r0_Egg, last_date )

last_date <- min(p0,n0,e0)
